				...
				while($teacher_code1!='')
				{
					...
					while($time!='')
					{
						...
						else
						{
							...
							{
								$room = empty_room($con,$st1,$et1,$day,'pr',$teacher_id,$shift,$subject_dept,$subject_dept2,$sem);
								if($room!=0)
								{
									$room_id=$room;
									$fill='blank';
			...
			while($teacher_code1!='')
			{
				...
				{ 
					...
					//	echo " >>>>>.....TH ".strtoupper($subject_year)."".strtoupper($shift)." st1=$st1 et1=$et1 day=$day";	
						...
						{
							if($st1!=17)
								$next=sidePrTh($con,$st1+1,$et1+1,$day,$shift,$subject_year,$subject_dept,$subject_dept2,'th',$sem);
							else
								$next=true;
							if($st1!=8)
								$prev=sidePrTh($con,$st1-1,$et1-1,$day,$shift,$subject_year,$subject_dept,$subject_dept2,'th',$sem);
							else
								$prev=true;
							if($prev==true and $next==true)
							{
								$room=empty_room2($con,$st1,$et1,$day,$subject_type,$teacher_id,$shift,$subject_dept,$subject_dept2,$sem);
								if($room!=0)
								{
	...
	if($type1=='pr' or $type1=='blank')
	{
		...
		{	
			if($row['type']=='pr')
			{
				...
				pr($con,$row['sr_no'],$id2,$st,$day,$row['teacher_id'],$row['subject_id'],0,$row['year'],$row['shift'],$row['batch'],$dept,$dept2,$sem);
			}
			else if($row['type']=='th')
			{
				...
				$temp_st=even_odd($con,$st);
				$temp_et=$temp_st+1;
				th($con,$temp_st,$temp_et,$day,$row['subject_id'],$row['teacher_id'],0,$row['year'],$row['shift'],$dept,$dept2,'update',$row['sr_no']);
				...
				{	
					...
					{
						//echo "---> TH ".strtoupper($row['year'])."".strtoupper($row['shift'])." st=".$st." et".$et." day".$day." ";
						th($con,$st,$et,$day,$row['subject_id'],$row['teacher_id'],0,$row['year'],$row['shift'],$dept,$dept2,'update',$row['sr_no']);
					}
				}
			}
		}
...
function replace_pr($con,$st,$et,$day,$teacher_id,$room_id,$year,$shift,$batch,$type1,$type2,$dept,$dept2,$sem)
{
	...
	while($row=mysqli_fetch_array($q))
	{
		...
		pr($con,$row['sr_no'],$id2,$st,$day,$row['teacher_id'],$row['subject_id'],0,$row['year'],$row['shift'],$row['batch'],$dept,$dept2);
	}
}
//--------------------------------------------------------------------------------------------------------
