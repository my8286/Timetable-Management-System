...
function checkbox()
{
	var chksA = document.getElementById("checkBatchA");
	var chksB = document.getElementById("checkBatchB");
	var chksC = document.getElementById("checkBatchC");

	if (chksA.checked) {
		return true;
	}
	else if (chksB.checked) {
		return true;
	}
...
{  
	...
	//var dept = deptVal();//document.getElementById("dept");  
							...
							{  
								...
								{ 
									...
									{ 
										...
										d=deptVal();e=email.value;p=num.value;de=des.value;
...
{
	var dept = deptVal();//document.getElementById("sdept"); 
	//alert(dept);
					...
					{	
						if(select(year," subject yer "))  
						{
							if(select(sem," semester "))  
							{
								...
								{*/
									insertSubject(sn.value,sc.value,st.value,th.value,pr.value,year.value,dept,sem.value);
...
{
	var dept = deptVal();//document.getElementById("rdept"); 
	...
	if(inputBoth(room," room "))  
	{
		if(select(type," room type "))  
		{
			...
			{*/
				insertRoom(room.value,dept,type.value);
...
{
	var dept = document.getElementById("udept"); 
	var username = document.getElementById("un");
	var pass = document.getElementById("pass"); 
	var udes = document.getElementById("udes");
	if(inputBoth(username," user name "))  
	{
		if(inputBoth(pass," password "))  
		{
			if(select(udes," user designation "))  
			{
				if(select(dept," user designation "))  
				{
					alert("user insertion complte");
				}
			}
		}
	}
...
//--------------------------------------------------------
document.getElementById('desHOD').style.display ='none';
document.getElementById('desPrin').style.display ='none';
function selectDesignation()
{
	...
	if (this.readyState == 4 && this.status == 200) {
		...
		if(this.responseText=='yes')
			document.getElementById('desHOD').style.display ='block';
		else
			document.getElementById('desHOD').style.display ='none';
	}
	};
	xhttp.open("GET", "../../plugins/timetable/form.php?checkDesignation=1&&dept="+dept, true);
	xhttp.send();
}
//------------------------------------------------------------
